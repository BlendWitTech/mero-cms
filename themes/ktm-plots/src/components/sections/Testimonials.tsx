  ...
  return (
    <div style={{ display: 'flex', gap: '2px' }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} width="16" height="16" fill={i < rating ? '#CC1414' : '#E5E7EB'} viewBox="0 0 24 24">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
          {list.map((t) => {
            ...
            return (
              ...
              >
                {/* Quote icon */}
                <svg width="28" height="28" fill="#CC1414" opacity="0.4" viewBox="0 0 24 24" style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  >
                    {avatarUrl ? (
                      ...
                      <span style={{ color: '#CC1414', fontWeight: 700, fontSize: '0.85rem' }}>{initials}</span>
                    )}
                  </div>
                  <div>
                    <div style={{ color: '#FFFFFF', fontWeight: 600, fontSize: '0.9rem' }}>{t.name}</div>
                    {t.role && <div style={{ color: '#A0A0A0', fontSize: '0.78rem' }}>{t.role}</div>}
                  </div>
                </div>
              </div>
            );
