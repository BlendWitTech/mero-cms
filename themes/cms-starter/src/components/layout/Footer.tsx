...
export default function Footer({ siteData }: FooterProps) {
  ...
  const footerLinks = footerMenu?.items
    ? [...footerMenu.items].sort((a, b) => a.order - b.order)
    : [];

  const year = new Date().getFullYear();
  const siteTitle = settings.siteTitle || 'Mero CMS';
  const copyrightText =
    settings.copyrightText || `© ${year} ${siteTitle}. All rights reserved.`;
    ...
    <footer className="bg-gray-900 text-gray-300">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-gray-400">
              ...
              {!settings.contactEmail && !settings.contactPhone && !settings.address && (
                <li className="text-gray-500 italic">No contact info configured.</li>
              )}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <div className="text-white font-semibold text-sm">Mero CMS</div>
                  <div className="text-gray-400 text-xs">Open Source</div>
                </div>
              </div>
              <p className="text-xs text-gray-400 leading-relaxed mb-3">
                This website is powered by Mero CMS — the flexible open-source content platform.
              </p>
              <a
                href="https://github.com/BlendWitTech/blendwit-cms-saas"
                target="_blank"
                rel="noopener noreferrer"
      ...
      <div className="border-t border-gray-800">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-gray-500 text-center sm:text-left">{copyrightText}</p>
          {settings.footerText && (
