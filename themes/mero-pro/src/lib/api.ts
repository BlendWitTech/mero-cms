...
/**
 * Active theme config — surfaces the resolved theme.json (with the
 * active design's `pageSchema` / `widgetCatalog` already merged on by
 * the backend's mergeActiveDesign helper). The runtime reads
 * `activeDesign` from here to set `<body data-design>` and dispatch
 * the home page through the matching design's widget-registry.
 *
 * Backend offline → null; layout falls back to `bundle.activeDesign`
 * from theme.json or just renders the default design.
 */
...
export function getActiveThemeConfig(): Promise<ActiveThemeConfig | null> {
    // Always fetch fresh — `bundle.activeDesign` can change at any
    // moment via the admin design picker, and a stale cached config
    // would leave the public site rendering the previous design even
    // after the customer flipped designs. The cost is one round-trip
    // per request; the saved bytes from caching aren't worth a
    // visibly-wrong design.
    return apiFetch<ActiveThemeConfig | null>('/themes/active/config', null, { revalidate: 0 });
}
